...
module.exports = async (deployer, network, accounts) => {
    ...
    let CrosspolyProxyV2Address = CONFIG.CrosspolyV2Proxy;
    ...
    if (deploySwitch.MOCK_V2_POOL) {
        ...
        var tx;
        {//Approve when change CrosspolyApprove Address
            ...
            // tx = await token0.approve(CrosspolyApproveAddress, "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
            // logger.log("Approve:" + token0Addr + " Tx:", tx.tx);
            // tx = await token1.approve(CrosspolyApproveAddress, "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
            // logger.log("Approve:" + token1Addr + " Tx:", tx.tx);
            tx = await weth.approve(CrosspolyApproveAddress, "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
            logger.log("Approve:" + token1Addr + " Tx:", tx.tx);
            // tx = await token2.approve(CrosspolyApproveAddress, "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
            // logger.log("Approve:" + token2Addr + " Tx:", tx.tx);
            // tx = await token3.approve(CrosspolyApproveAddress, "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
            // logger.log("Approve:" + token3Addr + " Tx:", tx.tx);
            // tx = await token4.approve(CrosspolyApproveAddress, "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
            // logger.log("Approve:" + token4Addr + " Tx:", tx.tx);
            // tx = await token5.approve(CrosspolyApproveAddress, "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
            // logger.log("Approve:" + token5Addr + " Tx:", tx.tx);
            // tx = await quote0.approve(CrosspolyApproveAddress, "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
            // logger.log("Approve:" + quote0Addr + " Tx:", tx.tx);
            // tx = await quote1.approve(CrosspolyApproveAddress, "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
            // logger.log("Approve:" + quote1Addr + " Tx:", tx.tx);
        }
        const CrosspolyProxyV2Instance = await CrosspolyProxyV2.at(CrosspolyProxyV2Address);
        ...
        //     tx = await CrosspolyProxyV2Instance.createCrosspolyVendingMachine(
        ...
        //DPP Pool
        for (var i = 0; i < POOL_PARAM.length; i++) {
            tx = await CrosspolyProxyV2Instance.createCrosspolyPrivatePool(
                POOL_PARAM[i].baseAddr,
                POOL_PARAM[i].quoteAddr,
                baseInAmount,
                quoteInAmount,
    ...
    if (deploySwitch.MOCK_TOKEN) {
        ...
        for (let i = 0; i < 0; i++) {
            // var tx = await ERC20FactoryInstance.createStdERC20(totalSupply, 'Crosspoly Bird', 'Crosspoly', 18);
            var tx = await ERC20FactoryInstance.createMintableERC20(totalSupply, 'Crosspoly Bird', 'Crosspoly', 18);
            // var tx = await ERC20FactoryInstance.createStdERC20(totalSupply, 'USDT Token', 'USDT', 6);
            logger.log("ERC20 address: ", tx.logs[0].args['erc20'] + "; Symbol:" + 'Crosspoly');
