...
contract CrosspolyCirculationHelper is InitializableOwnable {
    using SafeMath for uint256;

    // ============ Storage ============

    address immutable _VCrosspoly_TOKEN_;
    address immutable _Crosspoly_TOKEN_;
    ...
    constructor(address vDodoToken,address crosspolyToken) public {
        _VCrosspoly_TOKEN_ = vDodoToken;
    ...
    function getCirculation() public view returns (uint256 circulation) {
        ...
        for (uint256 i = 0; i < _LOCKED_CONTRACT_ADDRESS_.length; i++) {
            circulation -= IERC20(_Crosspoly_TOKEN_).balanceOf(_LOCKED_CONTRACT_ADDRESS_[i]);
        }
    }

    function getDodoWithdrawFeeRatio() external view returns (uint256 ratio) {
        uint256 crosspolyCirculationAmout = getCirculation();
        uint256 x =
            DecimalMath.divCeil(
                IERC20(_VCrosspoly_TOKEN_).totalSupply() * 100,
                crosspolyCirculationAmout
            );
