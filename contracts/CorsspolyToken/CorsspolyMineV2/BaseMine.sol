...
contract BaseMine is InitializableOwnable {
    ...
    function getPendingReward(address user, uint256 i) public view returns (uint256) {
        require(i<rewardTokenInfos.length, "CrosspolyMineV2: REWARD_ID_NOT_FOUND");
    ...
    function getIdByRewardToken(address rewardToken) public view returns(uint256) {
        ...
        require(false, "CrosspolyMineV2: TOKEN_NOT_FOUND");
    }
    ...
    function addRewardToken(
        ...
        require(rewardToken != address(0), "CrosspolyMineV2: TOKEN_INVALID");
        require(startBlock > block.number, "CrosspolyMineV2: START_BLOCK_INVALID");
        require(endBlock > startBlock, "CrosspolyMineV2: DURATION_INVALID");

        uint256 len = rewardTokenInfos.length;
        for (uint256 i = 0; i < len; i++) {
            require(
                rewardToken != rewardTokenInfos[i].rewardToken,
                "CrosspolyMineV2: TOKEN_ALREADY_ADDED"
            );
        }
        ...
        onlyOwner
    {
        ...
        require(block.number < newEndBlock, "CrosspolyMineV2: END_BLOCK_INVALID");
        require(block.number > rt.startBlock, "CrosspolyMineV2: NOT_START");
        require(block.number < rt.endBlock, "CrosspolyMineV2: ALREADY_CLOSE");
    ...
    function withdrawLeftOver(uint256 i, uint256 amount) external onlyOwner {
        ...
        RewardTokenInfo storage rt = rewardTokenInfos[i];
        require(block.number > rt.endBlock, "CrosspolyMineV2: MINING_NOT_FINISHED");
