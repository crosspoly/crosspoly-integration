...
contract CrosspolyV2RouteHelper {
    ...
    function getPairDetail(address token0,address token1,address userAddr) external view returns (PairDetail[] memory res) {
        (address[] memory baseToken0DVM, address[] memory baseToken1DVM) = ICrosspolyV2(_DVM_FACTORY_).getCrosspolyPoolBidirection(token0,token1);
        (address[] memory baseToken0DPP, address[] memory baseToken1DPP) = ICrosspolyV2(_DPP_FACTORY_).getCrosspolyPoolBidirection(token0,token1);
        (address[] memory baseToken0DSP, address[] memory baseToken1DSP) = ICrosspolyV2(_DSP_FACTORY_).getCrosspolyPoolBidirection(token0,token1);
        ...
        for(uint8 i = 0; i < len; i++) {
            ...
            (            
                ...
                curRes.Q0,
                curRes.R
            ) = ICrosspolyV2(cur).getPMMStateForCall();

            (curRes.lpFeeRate, curRes.mtFeeRate) = ICrosspolyV2(cur).getUserFeeRate(userAddr);
            curRes.curPair = cur;
            res[i] = curRes;
        }
    }
