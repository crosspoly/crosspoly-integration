...
import {IDVM} from "../../CrosspolyVendingMachine/intf/IDVM.sol";
import {IFragment} from "../../GeneralizedFragment/intf/IFragment.sol";
import {ICrosspolyNFTRegistry} from "../../Factory/Registries/CrosspolyNFTRegistry.sol";
...
/**
 * @title CrosspolyNFTProxy
 * @author Crosspoly Breeder
 *
 * @notice Entrance of NFT in Crosspoly platform
 */
contract CrosspolyNFTProxy is ReentrancyGuard, InitializableOwnable {
    ...
    modifier judgeExpired(uint256 deadLine) {
        require(deadLine >= block.timestamp, "CrosspolyNFTProxy: EXPIRED");
        _;
    }
    ...
    function createFragment(
        ...
        ICrosspolyNFTRegistry(_NFT_REGISTY_).addRegistry(msg.sender, newFragment, _quoteToken, newDvm);
    ...
    function buyout(
        ...
        if(flag == 0)
            require(msg.value == 0, "CrosspolyNFTProxy: WE_SAVED_YOUR_MONEY");
        ...
        require(curRequireQuote <= quoteMaxAmount, "CrosspolyNFTProxy: CURRENT_TOTAL_VAULE_MORE_THAN_QUOTEMAX");

        _deposit(msg.sender, fragment, IFragment(fragment)._QUOTE_(), curRequireQuote, flag == 1);
        IFragment(fragment).buyout(msg.sender);

        ICrosspolyNFTRegistry(_NFT_REGISTY_).removeRegistry(fragment);
