...
import {ICrosspolyCallee} from "../../intf/ICrosspolyCallee.sol";
...
contract Fragment is InitializableFragERC20 {
    ...
    function init(
      ...
      string memory _symbol
        ...
        require(!_FRAG_INITIALIZED_, "CrosspolyFragment: ALREADY_INITIALIZED");
        ...
        // init FRAG meta data
        name = string(abi.encodePacked("Crosspoly_FRAG_", _symbol));
    ...
    function buyout(address newVaultOwner) external {
        require(_BUYOUT_TIMESTAMP_ != 0, "CrosspolyFragment: NOT_SUPPORT_BUYOUT");
        require(block.timestamp > _BUYOUT_TIMESTAMP_, "CrosspolyFragment: BUYOUT_NOT_START");
        require(!_IS_BUYOUT_, "CrosspolyFragment: ALREADY_BUYOUT");

        int buyoutFee = IBuyoutModel(_BUYOUT_MODEL_).getBuyoutStatus(address(this), newVaultOwner);
        require(buyoutFee != -1, "CrosspolyFragment: USER_UNABLE_BUYOUT");
        ...
        require(payQuote >= requireQuote, "CrosspolyFragment: QUOTE_NOT_ENOUGH");
    ...
    function redeem(address to, bytes calldata data) external {
        require(_IS_BUYOUT_, "CrosspolyFragment: NEED_BUYOUT");
        ...
        if (data.length > 0) {
          ICrosspolyCallee(to).NFTRedeemCall(
            msg.sender,
            quoteAmount,
            data
          );
