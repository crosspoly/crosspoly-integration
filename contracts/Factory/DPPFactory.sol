...
import {IDPP} from "../CrosspolyPrivatePool/intf/IDPP.sol";
import {IDPPAdmin} from "../CrosspolyPrivatePool/intf/IDPPAdmin.sol";

/**
 * @title Crosspoly PrivatePool Factory
...
contract DPPFactory is InitializableOwnable {
    ...
    function createCrosspolyPrivatePool() external returns (address newPrivatePool) {
        newPrivatePool = ICloneFactory(_CLONE_FACTORY_).clone(_DPP_TEMPLATE_);
    }

    function initCrosspolyPrivatePool(
        address dppAddress,
        address creator,
        address baseToken,
        address quoteToken,
    ...
    function _createDPPAdminModel(
        ...
        IDPPAdmin(adminModel).init(owner, dpp, operator, crosspolyApproveProxy);
    }
    ...
    function getCrosspolyPool(address baseToken, address quoteToken)
        external
        view
    ...
    function getCrosspolyPoolBidirection(address token0, address token1)
        external
        view
    ...
    function getCrosspolyPoolByUser(address user) 
        external
        view
        returns (address[] memory pools)
    {
