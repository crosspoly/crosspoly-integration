...
contract CrosspolyNFT1155 is ERC1155, InitializableOwnable {
    ...
    // ============ Event =============
    event CrosspolyNFTMint(address creator, uint256 tokenId, uint256 amount);
    event CrosspolyNFTBurn(address account, uint256 tokenId, uint256 amount);
    ...
    function mint(string calldata uri, uint256 amount) external {
        ...
        emit CrosspolyNFTMint(msg.sender, _CUR_TOKENID_, amount);
    ...
    function burn(address account, uint256 tokenId, uint256 amount) external onlyOwner {
        ...
        emit CrosspolyNFTBurn(account, tokenId, amount);
    }
