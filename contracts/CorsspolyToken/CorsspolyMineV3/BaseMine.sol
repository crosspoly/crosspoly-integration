...
contract BaseMine is InitializableOwnable {
    ...
    function getPendingReward(address user, uint256 i) public view returns (uint256) {
        require(i<rewardTokenInfos.length, "CrosspolyMineV3: REWARD_ID_NOT_FOUND");
    ...
    function getIdByRewardToken(address rewardToken) public view returns(uint256) {
        ...
        require(false, "CrosspolyMineV3: TOKEN_NOT_FOUND");
    }
    ...
    function addRewardToken(
        ...
        require(rewardToken != address(0), "CrosspolyMineV3: TOKEN_INVALID");
        require(startBlock > block.number, "CrosspolyMineV3: START_BLOCK_INVALID");
        require(endBlock > startBlock, "CrosspolyMineV3: DURATION_INVALID");

        uint256 len = rewardTokenInfos.length;
        for (uint256 i = 0; i < len; i++) {
            require(
                rewardToken != rewardTokenInfos[i].rewardToken,
                "CrosspolyMineV3: TOKEN_ALREADY_ADDED"
            );
        }
        ...
        onlyOwner
    {
        ...
        require(totalDepositReward >= totalReward, "CrosspolyMineV3: REWARD_NOT_ENOUGH");

        require(block.number < newEndBlock, "CrosspolyMineV3: END_BLOCK_INVALID");
        require(block.number > rt.startBlock, "CrosspolyMineV3: NOT_START");
        require(block.number < rt.endBlock, "CrosspolyMineV3: ALREADY_CLOSE");
    ...
    function withdrawLeftOver(uint256 i, uint256 amount) external onlyOwner {
        ...
        RewardTokenInfo storage rt = rewardTokenInfos[i];
        require(block.number > rt.endBlock, "CrosspolyMineV3: MINING_NOT_FINISHED");
    ...
    function directTransferOwnership(address newOwner) external onlyOwner {
        require(newOwner != address(0), "CrosspolyMineV3: ZERO_ADDRESS");
