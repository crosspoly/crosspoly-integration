...
/**
 * @title CrosspolyMigration between Ethereum and BSC
 * @author Crosspoly Breeder
 */
contract CrosspolyMigrationBSC is InitializableOwnable {
    using SafeMath for uint256;

    // ============ Storage ============

    address public immutable _ETH_Crosspoly_TOKEN_;
    address public immutable _Crosspoly_APPROVE_PROXY_;
    mapping(address => uint256) public balances;

    constructor(address ethDodoToken, address crosspolyApproveProxy) public {
        _ETH_Crosspoly_TOKEN_ = ethDodoToken;
    ...
    function unlock(address unlockTo, uint256 amount) external onlyOwner {
        ...
        IERC20(_ETH_Crosspoly_TOKEN_).transfer(unlockTo, amount);
