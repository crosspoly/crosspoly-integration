...
contract CrosspolyNFTRouteHelper {
    ...
    function getPairDetail(address token0,address token1,address userAddr) external view returns (PairDetail[] memory res) {
        (address[] memory baseToken0DVM, address[] memory baseToken1DVM) = ICrosspolyV2(_NFT_REGISTER_).getCrosspolyPoolBidirection(token0,token1);
