...
import {ICrosspolyRewardVault, CrosspolyRewardVault} from "./CrosspolyRewardVault.sol";

contract CrosspolyMine is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Crosspolys
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accCrosspolyPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accCrosspolyPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Crosspolys to distribute per block.
        uint256 lastRewardBlock; // Last block number that Crosspolys distribution occurs.
        uint256 accCrosspolyPerShare; // Accumulated Crosspolys per share, times 1e12. See below.
    }

    address public crosspolyRewardVault;
    ...
    // The block number when Crosspoly mining starts.
    ...
    constructor(address _crosspolyToken, uint256 _startBlock) public {
        crosspolyRewardVault = address(new CrosspolyRewardVault(_crosspolyToken));
    ...
    function setReward(uint256 _crosspolyPerBlock, bool _withUpdate) external onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function getPendingReward(address _lpToken, address _user) external view returns (uint256) {
        ...
        uint256 accCrosspolyPerShare = pool.accCrosspolyPerShare;
        uint256 lpSupply = IERC20(pool.lpToken).balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 CrosspolyReward = block
                .number
                .sub(pool.lastRewardBlock)
                .mul(crosspolyPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accCrosspolyPerShare = accCrosspolyPerShare.add(DecimalMath.divFloor(CrosspolyReward, lpSupply));
        }
        return DecimalMath.mulFloor(user.amount, accCrosspolyPerShare).sub(user.rewardDebt);
    }
    ...
    function getAllPendingReward(address _user) external view returns (uint256) {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            totalReward = totalReward.add(
                DecimalMath.mulFloor(user.amount, accCrosspolyPerShare).sub(user.rewardDebt)
            );
        }
        return totalReward;
    }
    ...
    function getDlpMiningSpeed(address _lpToken) external view returns (uint256) {
        uint256 pid = getPid(_lpToken);
        PoolInfo storage pool = poolInfos[pid];
        return crosspolyPerBlock.mul(pool.allocPoint).div(totalAllocPoint);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 CrosspolyReward = block
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        pool.accCrosspolyPerShare = pool.accCrosspolyPerShare.add(DecimalMath.divFloor(CrosspolyReward, lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(address _lpToken, uint256 _amount) public {
        ...
        updatePool(pid);
        if (user.amount > 0) {
            uint256 pending = DecimalMath.mulFloor(user.amount, pool.accCrosspolyPerShare).sub(
                user.rewardDebt
            );
            safeCrosspolyTransfer(msg.sender, pending);
        }
        IERC20(pool.lpToken).safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = DecimalMath.mulFloor(user.amount, pool.accCrosspolyPerShare);
    ...
    function claimAll() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            updatePool(pid);
            pending = pending.add(
                DecimalMath.mulFloor(user.amount, pool.accCrosspolyPerShare).sub(user.rewardDebt)
            );
            user.rewardDebt = DecimalMath.mulFloor(user.amount, pool.accCrosspolyPerShare);
        }
        safeCrosspolyTransfer(msg.sender, pending);
    ...
    // Safe Crosspoly transfer function
    function safeCrosspolyTransfer(address _to, uint256 _amount) internal {
        ICrosspolyRewardVault(crosspolyRewardVault).reward(_to, _amount);
