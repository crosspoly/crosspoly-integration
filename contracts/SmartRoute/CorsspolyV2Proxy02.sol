...
/**
 * @title CrosspolyV2Proxy02
 * @author Crosspoly Breeder
 *
 * @notice Entrance of trading in Crosspoly platform
 */
contract CrosspolyV2Proxy02 is ICrosspolyV2Proxy01, ReentrancyGuard, InitializableOwnable {
    ...
    modifier judgeExpired(uint256 deadLine) {
        require(deadLine >= block.timestamp, "CrosspolyV2Proxy02: EXPIRED");
        _;
    }
        ...
        )
    {
        ...
        require(
            ...
            "CrosspolyV2Proxy02: deposit amount is not enough"
        );
    ...
    {
        require(crosspolyPairs.length > 0, "CrosspolyV2Proxy02: PAIRS_EMPTY");
        require(minReturnAmount > 0, "CrosspolyV2Proxy02: RETURN_AMOUNT_ZERO");
        ...
        require(returnAmount >= minReturnAmount, "CrosspolyV2Proxy02: Return amount is not enough");
        ...
        returns (uint256 returnAmount)
    {
        ...
        ICrosspolyApproveProxy(_Crosspoly_APPROVE_PROXY_).claimTokens(fromToken, msg.sender, crosspolyPairs[0], fromTokenAmount);
        ...
        returns (uint256 returnAmount)
    {
        ...
        require(isWhiteListed[swapTarget], "CrosspolyV2Proxy02: Not Whitelist Contract");
        (bool success, ) = swapTarget.call{value: fromToken == _ETH_ADDRESS_ ? msg.value : 0}(callDataConcat);

        require(success, "CrosspolyV2Proxy02: External Swap execution Failed");
    ...
    function mixSwap(
        ...
        require(mixPairs.length > 0, "CrosspolyV2Proxy02: PAIRS_EMPTY");
        require(mixPairs.length == mixAdapters.length, "CrosspolyV2Proxy02: PAIR_ADAPTER_NOT_MATCH");
        require(mixPairs.length == assetTo.length - 1, "CrosspolyV2Proxy02: PAIR_ASSETTO_NOT_MATCH");
        require(minReturnAmount > 0, "CrosspolyV2Proxy02: RETURN_AMOUNT_ZERO");
    ...
    function addLiquidityToV1(
        ...
        require(baseShares >= baseMinShares && quoteShares >= quoteMinShares,"CrosspolyV2Proxy02: Return DLP is not enough");
    }
