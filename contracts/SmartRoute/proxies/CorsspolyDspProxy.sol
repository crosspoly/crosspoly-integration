...
import {IDSP} from "../../CrosspolyStablePool/intf/IDSP.sol";
import {IDSPFactory} from "../../Factory/DSPFactory.sol";

/**
 * @title CrosspolyDspProxy
 * @author Crosspoly Breeder
 *
 * @notice Entrance of Crosspoly Stable Pair in Crosspoly platform
 */
contract CrosspolyDspProxy is ReentrancyGuard {
    ...
    modifier judgeExpired(uint256 deadLine) {
        require(deadLine >= block.timestamp, "CrosspolyDspProxy: EXPIRED");
        _;
    }
    ...
    function createCrosspolyStablePair(
        address baseToken,
        address quoteToken,
        ...
        returns (address newCrosspolyStablePair, uint256 shares)
    {
        {
            address _baseToken = baseToken == _ETH_ADDRESS_ ? _WETH_ : baseToken;
            address _quoteToken = quoteToken == _ETH_ADDRESS_ ? _WETH_ : quoteToken;
            newCrosspolyStablePair = IDSPFactory(_DSP_FACTORY_).createCrosspolyStablePool(
                _baseToken,
                _quoteToken,
                lpFeeRate,
                i,
        ...
        (shares, , ) = IDSP(newCrosspolyStablePair).buyShares(msg.sender);
    }
        ...
        )
    {
        ...
        require(
            ...
            "CrosspolyDspProxy: deposit amount is not enough"
        );
